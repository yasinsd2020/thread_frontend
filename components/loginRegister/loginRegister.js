...
const loginRegister = ({
  ...
  return (
    <>
      <div
        className={`fixed ${
          loginPopup?.login ? "right-0" : "-right-[100%]"
        } w-[100%] h-screen ease-linear duration-150`}
        style={{ zIndex: 1000, background: "rgba(0,0,0,0.5)" }}
      >
        <div
          className="relative  md:-right-[33vw] md:top-[10vh] bg-white md:w-[35%] w-[100%] 
        ...
        >
          <div className="w-full flex justify-between items-center mb-[8vh] md:mb-[2vh]">
            <article className="font-bold font-diot text-xl"></article>
            <div
          ...
          <div className="flex justify-center gap-3 items-center w-full">
            <div
              className={`font-normal cursor-pointer text-lg ${
                loginPopup?.register ? "text-gray-400	" : "text-black"
              }`}
              onClick={() => {
                setLoginPopup((prev) => {
            ...
            <div
              className={`font-normal cursor-pointer text-lg ${
                loginPopup?.register ? "text-black  " : "text-gray-400 	"
              }`}
              onClick={() => {
                setLoginPopup((prev) => {
          ...
          <div className="w-full flex flex-col mt-4 gap-8">
            <div>
              {loginPopup?.register ? (
                <div className="flex flex-col gap-3">
                  <Input
                    customCss={"md:w-full border-black w-full"}
                    name="first_name"
            ...
            <div>
              <CommonButton
                text="SUBMIT"
                onClick={() => submitFun()}
                customClass={"md:w-full w-full cursor-pointer"}
              />
            </div>
          </div>
        </div>
